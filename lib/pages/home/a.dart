...
class _LivecoursePageState extends State<LivecoursePage> with SingleTickerProviderStateMixin {
  ...
  void getLogo() async {
    ...
    setState(() {
      ...
      userNme = json.decode(userinfo)['UserName'];
    });
  }
  ...
  getCtRoomService() {
    var action = (CtRoomService data) {
      ...
      data.msgStream.listen((msg){
        ...
        setState(() {
          _getchatroomList.add({'Content': msg.msg, "IsSelf": false, 'logoUrl': msg.logoUrl, 'UserNme': msg.name});
        });
         _scrollController.animateTo(_scrollController.position.maxScrollExtent,
        duration: new Duration(seconds: 2), curve: Curves.ease);
      }); // 接收消息
  ...
  void sendM(){
    ...
    setState(() {
      _getchatroomList.add({'Content': _inputValue, "IsSelf": true, 'logoUrl': logoUrl, 'UserNme': userNme});
    });
              ...
              return Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    SizedBox(width: ScreenAdaper.width(item['IsSelf']?0:10),),
                    Expanded(
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: item['IsSelf'] ? CrossAxisAlignment.end : CrossAxisAlignment.start,
                        children: <Widget>[
                          Text('${item['UserName']}', style: TextStyle(color: Color(item['IsSelf']?0xffFF8636 :0xff000000), fontSize: ScreenAdaper.size(26)),),
                          SizedBox(height: ScreenAdaper.height(5),),
                          Container(
                            padding: EdgeInsets.symmetric(horizontal: ScreenAdaper.width(27), vertical: ScreenAdaper.height(25)),
                            decoration: BoxDecoration(
                    ...
                    SizedBox(width: ScreenAdaper.width(item['IsSelf']?10:0),),
